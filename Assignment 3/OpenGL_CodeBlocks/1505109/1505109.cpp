...
inline void Refract(VEC3 &out, const VEC3 &incidentVec, const VEC3 &normal, float eta)
{
...
public:
    ...
    Point getRefraction(Point ray_dir, Point normal)
    {
        double ndot = ray_dir.dot(normal);
        double k = 1.0 - e * e * (1.0 - ndot * ndot);
        if (k < 0.0)
            return Point(0.0, 0.0, 0.0);

        Point refraction = ray_dir*e - normal*(e * ndot + sqrt(k));
    ...
    color updatedColor(Point ray_start, Point dir,int level, int objid)
    {
        ...
        Point refractedL=getRefraction(secting-ray_start,normal);

        double lambert = 0.2, specular = 0.4;

        for(int i=0; i<lights.size(); i++)
        {

            Point dirtoLight=ray_start-lights[i].position;
            Point start = lights[i].position;

            Ray L(start, dir);
            Point R = (normal * (L.dir.dot(normal) * 2.0) - L.dir).normalize();
            Point V = (secting * -1.0).normalize();
            ...
            if(!flag)
            {
                double specular = coefficients.specular * pow(R.dot(V),shine);
                clip(specular,0.0,1.0);
                res.g+=specular*clr.g;
                res.b+=specular*clr.b;
                res.r+=specular*clr.r;
            }
            /*
            if (level < recursion_level)
            {
            Point3 start = intersectionPoint + reflection;
...
void readData()
{
    ...
    fovY = 80;
    aspect_ratio = 1;
    description>> reccLevel >>imageWidth;
    ...
    {
        Lights l;
        description>>l.position.x>>l.position.y>>l.position.z;
